...
%%
%create Fig. 2 - Fig. supplement 6 - regression on reward probabilities
%(i.e., objective action-values)
alpha=2;
...
mSize=2.5;
Draw_scatter_plot(reg.random_walk(samVec,:),alpha,mSize,[ 0.1    0.6300    0.24    0.32],'random walk');
mTextBox = text(-0.4,1.1,'A','interpreter','tex',...
    'Units','Normalized','FontSize',22,'Color','k','FontWeight','bold');

%plot bar chart - RW
sample_size=1000;
Draw_four_bar_plots(reg.random_walk,alpha,sample_size,[ 0.35    0.63    0.14    0.32],0,[-0.1 0 0.2 0.2]);


%create plots for reg on reward prboabilities - motor_cortex data
i=14;
...
%create plots for reg on reward prboabilities  - auditory_cortex data
i=20;
...
Draw_four_bar_plots(regForPlot,alpha,sample_size,[ 0.35    0.13    0.14    0.32],0,[-0.1 0 0.2 0.2]);

%create plots for reg on reward prboabilities - basal_ganglia data
if ~exist('basal_ganglia_data.mat','file')
    disp('basal ganglia data is missing')
else
i=29;
...
sample_size=floor(size(regForPlot,1)/40);
hold on
Draw_four_bar_plots(regForPlot,alpha,sample_size,[ 0.85    0.13    0.14    0.32],0,[-0.1 0 0.2 0.13]);
end
